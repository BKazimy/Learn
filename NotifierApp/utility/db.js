...
class QuoteDatabase {
  ...
  async SetQuoteOfDay() {
    try {
      let data = await this.getRandom();
      id = {id: data.id};
      // console.log('setquote id:', id);
      await AsyncStorage.setItem('today', JSON.stringify(id));
      return id;
    } catch (e) {
  ...
  async GetQuoteOfDay() {
    try {
      ...
      return jsonValue != null ? JSON.parse(jsonValue) : this.SetQuoteOfDay(); //this.SetQuoteOfDay();
    } catch(e) {
